...
public class ModalView : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        foreach (Modal it in modals) it.Cull(true);
    }
    ...
    public void Open(Modal p_modal)
    {
        ...
        //For frame holder only
        // public void Open(ModalType p_modal, string p_frame_name) {
        //     Open(p_modal);
        //     if (p_frame_name == "" || current == null) return;

        //     GameObject frameObject = Resources.Load<GameObject>("Prefab/Frame/" + p_frame_name);
        //     FrameHolderModal holder = (FrameHolderModal)current;
        //     holder.Set( frameObject);
        // }
    ...
    public void Close()
    {
        if(current)
        {
            //foreach(Modal it in opened) it.Close();
